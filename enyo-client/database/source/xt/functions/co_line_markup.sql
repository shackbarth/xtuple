  ...
  select
    case
      ...
      when fetchmetricbool('Long30Markups') then
        round((1.0 - (currtobase(cohead_curr_id, $1.coitem_price, cohead_orderdate) / $1.coitem_unitcost)) * 100.0, 6)
      else round(((currtobase(cohead_curr_id, $1.coitem_price, cohead_orderdate) / $1.coitem_unitcost) - 1.0) * 100, 6)
    end
