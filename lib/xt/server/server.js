...
XT.Server = XT.Object.extend(
  ...
  /**
    If this is set to true then the server
  ...
  */
  start: function() {
    ...
    try {
      ...
      if(w) {
        s._io = require('socket.io').listen(s.server, { 'log level': 0 });
        s._io.configure(function() { s._io.set('transports', ['websocket', 'xhr-polling']); });
...
XT.mixin(XT.Server,
  ...
  /**
    Collection of all available servers.
  */
  activeServers: {},
