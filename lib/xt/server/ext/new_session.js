...
XT.Session = XT.Object.extend(
  ...
  from: function(credentials) {
    ...
    if (username && organization && sid) {
      ...
      this.error = new SessionError(null, "missing credentials for session request");
  ...
  ready: function(action) {
    ...
    if (ready) {
      XT.debug("ready received and flushing");
      return action(this);
    }

    XT.debug("ready received and queuing");
         ...
         for(; i<l; ++i) {
           ...
           XT.cache.hgetall(sessionKey, function(e, data) {
             ...
             if (!XT.Session.isActiveSession(data)) {
               ...
               XT.debug("REMOVING %@ FROM CACHE".f(key));
