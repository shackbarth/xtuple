...
xt.sessionStore = xt.object.create(
  ...
  */
          ...
          for(; i<l; ++i) {
            ...
            xt.cache.hgetall(sessionKey, function(e, data) {
              ...
              if(dif > timeout) {
                ...
                xt.log("Removing expired session for user '%@' (%@)".f(user, data.sid));
                xt.cache.del(sessionKey);
              }
              
              else xt.log("%@ => %@ (%@)".f(data.sid, dif, timeout));
            });
  ...
  init: function() {
    ...
    this.__pollingTimer__ = setInterval(this.pollCache, 6000 /* 1 minute */);
  },
