...
XT.sessionStore = XT.Object.create(
  ...
  */
  createSession: function(xtr, callback) {
    ...
    XT.db.query(company, query, function(err, ret) {
      if(err) {
      ...
      if(ret.validate_user) {
        self.loadUser(userName, company, function(err, userkey) {
          ...
          // to free it
          var session = XT.Session.create({ userKey: userkey, company: company, xtr: xtr, userName: userName });
  ...
  */
  validateSession: function(xtr, callback) {
    ...
    var key = '%@:%@:%@:%@'.f(userName, ip, sid, company);
  ...
  */
  ...
  /**
    ...
    @method
  */
  loadUser: function(userName, company, callback, context, force) {
    ...
    XT.cache.hgetall(userKey, function(err, ret) {
      ...
      if (XT.none(ret) || Object.keys(ret).length == 0 || force) {
        ...
        XT.database.query(company, query, function(err, ret) {
          ...
          for(; i<l; ++i) {
            ...
            XT.cache.hgetall(sessionKey, function(e, data) {
              ...
              if(dif > timeout) {
                ...
                var key = '%@:%@:%@:%@'.f(data.userName, data.ip, data.sid, data.company);
                XT.cache.del(key);
              }
              else {
