...
XT.Router = XT.Object.extend(
  ...
  */
  route: function(xtr) {
    ...
    if(XT.none(r) || Object.keys(r).length <= 0)
      issue(XT.close(
...
// go ahead and find all of the available routers so the servers
// can use them when the framework is up
XT.run(function() {
  var files;
  var idx = 0;
  var file;
  ...
  XT.log("Loading available routers from %@".f(
    XT.shorten(_path.join(__dirname, '../routers'), 5)));

  // change directories to path relatively
  process.chdir(__dirname);

  // collect any available javascript files in the directory
  files = XT.directoryFiles('../routers', 'js');

  // require each of them
  for (; idx < files.length; ++idx) {
    file = files[idx];
    path = _path.join('../routers', file);
    require(path);
  }
});
