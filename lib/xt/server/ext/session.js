...
XT.Session = XT.Object.extend(
  ...
  init: function() {
    ...
    if(xtr.get('useSession') && !XT.none(handler)) {
      readyQueue.push(function() { handler.handle(xtr); });
    }
  ...
  */
  commit: function(callback) {
    ...
    this.data = data;
    key = '%@:%@:%@:%@'.f(data.userName, data.ip, data.sid, data.organization);
  ...
  query: function(query, callback) {
    ...
    XT.debug("Session about to execute query => %@".f(query));

    // we need to fire off the request via the database driver
    XT.database.query(organization, query, func);
  },
