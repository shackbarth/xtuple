  ...
  query: function(query) {
    ...
    XT.database.query(organization, query, function(err, ret) {
      if (err) {
        ...
        self.messageClient("query did error: " + err);  
  ...
  */
  ...
  pollCache: function() {
   ...
   XT.cache.keys('user:*', function(e, loadedUsers) {
     if(e) return issue(XT.warning("Could not poll cache for active users"));
     var i = 0,
     ...
     for(; i<l; ++i) {
       ...
       XT.cache.keys('%@:*'.f(user), function(e, sessionsFor) {
         if(e) return issue(XT.warning("Could not poll cache for active sessions"));
         var i = 0,
         ...
         for(; i<l; ++i) {
           ...
           XT.cache.hgetall(sessionKey, function(e, data) {
             if(e) return issue(XT.warning("Could not retrieve session data for entry while polling"));
...
XT.Session.poll = function() {
  ...
  XT.Session.sessionTimeout = XT.opts.datasource.sessionTimeout * 60000;

  // share this information with the world...  
  XT.log("Session timeout has been set to %@ minutes (%@ milliseconds)".f(XT.opts.datasource.sessionTimeout, XT.Session.sessionTimeout));
