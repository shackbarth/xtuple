...
XT.Response = XT.Object.extend(
  ...
  */
  ...
  /**
    ...
    Will be true unless the pipe has been closed. Subsequent
  ...
  */
  write: function() {
    ...
    for(; i<l; ++i) {     
      ...
      r.write(a[i]);
      XT.debug("Response writing => %@".f(a[i]));
    }
    return this;
  },
  ...
  */
  close: function() {
    if(this.get('isOpen') === false) return;
  ...
  */
  ...
  getSession: function(p, useSession) {
    ...
    // XT.debug("XT.Response.useSession %@".f(useSession ? "true" : "false"));
