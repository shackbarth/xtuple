...
XT.mixin(
  ...
  */
  ...
  /**
    ...
    @private
  */
  parseConfiguration: function parseConfiguration(file) {
    var config;

    // get the path to the configuration file
    file = _path.resolve(XT.basePath, file);
    
    try {
      
      // attempt to read the file contents
      config = _fs.readFileSync(file, 'utf8');
      
      // since we successfully retrieve the contents
      // go ahead and mix the json parsed file contents
      // into our globals opts property
      XT.mixin(XT.opts, XT.json(config, true));

    } catch (err) {
      issue(XT.fatal("Could not parse configuration file", err));
    }
  },
