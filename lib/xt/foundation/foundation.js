...
XT.mixin(
  ...
  */
  ...
  /**
    ...
    @todo Rewrite to use ASYNC module
    
    @param {String} path The path to the file relative to execution path.
    @private
  */
  parseConfiguration: function parseConfiguration(f) {
    f = _path.resolve(XT.basePath, f);
    XT.readFile(f, function(e, d) {
      if(e) throw XT.fatal(e);
      XT.mixin(XT.opts, XT.json(d));
      if(XT.opts.debug)
        XT.DEBUGGING = XT.opts.debug;
      XT.mode = XT.opts.mode;
      var checks = "encryptionKey modelsDir subModelsDir".w(),
          done = [],
          didFinish = false;
      process.on('xtConfigCheckDone', function(which) {
        var idx = checks.contains(which);
        idx = idx === true ? 0 : idx; // convert to zero if needed
        if(XT.none(idx)) {
          if(done.contains(which))
            return issue(XT.warning("WTF, %@ returned twice in startup?".f(which)));
          else return issue(XT.warning("WTF, %@ was sent back as a completed startup task?".f(which)));
        }
        checks.splice(idx, 1);
        done.push(which);
        if(checks.length == 0) {
          ...
          didFinish = true;
          process.removeAllListeners('xtConfigCheckDone');
        }
      });

      // we don't want this to hose forever if something is wrong...
      setTimeout(function() { 
        if(didFinish) return;
        var single = checks.length > 1 ? false : true;
        issue(XT.warning("Something is wrong, configuration setup shouldn't take this long. " +
          "There %@ still %@ check%@ left undone.".f((single ? 'is' : 'are'), checks.length, (single ? '' : 's'))));
        process.emit('SIGINT');
      }, 3000);

      // check encryptionKey status
      XT.readFile(XT.opts['encryption-key-file'], function(err, key) {
        if(err) throw XT.fatal(err);
        XT.ENCRYPTIONKEY = key.trim();
        process.emit('xtConfigCheckDone', 'encryptionKey');
      });

      // check modelsDir
      XT.exists(XT.opts['generated-models-directory'], function(exists, stats) {
        if(!exists)
          return issue(XT.fatal("Could not stat the generated models directory. " +
            "Check the configuration and ensure it has a valid entry."));
        // originally was going to try and do some elegant test for
        // permissions but in the end a brute force try-until-it-fails will
        // have to do for simplicity
        // var mode = stats.mode.toString(8).slice(2);
        try {
          var path = XT.opts['generated-models-directory'],
              file = 'testfile' + process.pid,
          path = _path.join(path, file);
          _fs.writeFileSync(path, '', 'utf8'); 
          _fs.unlinkSync(path);

          // ok it works
          process.emit('xtConfigCheckDone', 'modelsDir');
          XT.MODELSDIRECTORY = XT.opts['generated-models-directory'];
          process.once('xtBootstrapping', function() {
            XT.log("Using %@ for generated models".f(XT.opts['generated-models-directory']));
          });
        } catch(err) {

          // could not write to the requested directory
          issue(XT.fatal("Could not write to the generated models directory. " +
            "Check permissions for the path in the configuration file."));
        }
      });

      // check subModelsDir
      XT.exists(XT.opts['generated-subclass-models-directory'], function(exists, stats) {
        if(!exists) {
          // unlike with the generated models directory we try to create this one
          try {
            _fs.createDir(XT.opts['generated-subclass-models-directory'], function(err) {
              if(err) throw err;
              process.emit('xtConfigCheckDone', 'subModelsDir');
            });
          } catch(err) {
            issue(XT.fatal("Could not create the subclass models directory. Check " +
              "permissions on the base directory for the setting in the configuration."));
          }
        }
        else {
          process.emit('xtConfigCheckDone', 'subModelsDir');
          ...
          process.once('xtBootstrapping', function() {
            XT.log("Using %@ for generated sub-class models".f(XT.opts['generated-subclass-models-directory']));
          });
        }
      });
