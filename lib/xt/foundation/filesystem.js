...
XT.basePath = XT.fs.basePath = process.cwd();
...
XT.mixin(XT.filesystem, 
  ...
  */
  join: function() {
    return this.__path__.join.apply(null, arguments);
  },

  /**
  */
  exists: function(path, callback) {
    var fs = this.__fs__, stats;
    if(!XT.none(callback) && XT.typeOf(callback) === XT.T_FUNCTION)
      return fs.stat(path, function(err, stats) {
        if(err) callback(NO, null);
        else if(stats.isDirectory() || stats.isFile())
          callback(YES, stats);
        else callback(NO, stats);
      });
    try {
      stats = fs.statSync(path);
      return stats.isDirectory() || stats.isFile() ? YES : NO;
    } catch(err) {
