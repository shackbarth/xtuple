...
var server = XT.dataServer = XT.Server.create({
  ...
  makeHandler: function(callback, event, functor) {
    return function(wrapper, ack) {
      ...
      if (functor.needsSession) {
        session = XT.Session.create().from(wrapper);
      }
      ...
      if (session) {
        session.ready(function(session) {
          callback(payload, session, wrappedAck, event);
        });
      } else { callaback(payload, wrappedAck, event); }
    }
  }
});
...
function initClientSocket(socket) {
  ...
  for (; idx < keys.length; ++idx) {
    handles = keys[idx];
    handler = map[handles];
    handler = this.makeHandler(handler.handle, handles, handler);
