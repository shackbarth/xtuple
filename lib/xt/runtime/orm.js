  ...
  */
  ...
  generateModelFiles: function(socket, list, subclass) {
    ...
    for(i=0; i<list.length; ++i) {
      ...
      gen[filename] = models[list[i]].superclass || "// ERROR: COULD falseT FIND GENERATED MODEL FOR ORM %@".f(list[i]);
      ...
      function(err, file) {
        ...
        try {
          ...
          if(!ext) {
            if(XT.modelGenerator) {
              models["%@.%@".f(ns, type)] = {
                superclass: XT.modelGenerator.generate(json, true),
                subclass: XT.modelGenerator.generate(json, false)
              };
            }
          }
        } catch(err) {
