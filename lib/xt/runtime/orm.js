...
XT.orm = XT.Object.create(
  ...
  */
  ...
  depsFor: function(orms, orm, deps) {
    ...
    // the namespace is not public, xt or null
  ...
  calculateDeps: function(socket, orms) {
    ...
    for(i=0; i<ens.length; ++i) {
      var namespace = ens[i],
          names = XT.keys(exts[namespace]);
      for(j=0; j<names.length; ++j) {
        var name = names[j],
            ext = exts[namespace][name];
        this.depsFor(exts, ext);
      }
    }
  ...
  installList: function(socket, list) {
    ...
    if(XT.none(list) || list.length == 0) {
      if(socket.extensionsInstalled) {
        ...
        socket.extensionsInstalled = NO;
        XT.debug("all done");
        return;
      }

      socket.send("attempting to install any extensions");

      var exts = socket.extensions,
          ns = XT.keys(exts), i, j, self = this;
      ...
      for(i=0; i<ns.length; ++i) {
        ...
        // THIS ONLY WORKS WITH ONE NAMESPACE!!!

        process.on('ORMEXTENSIONINSTALLED', function() {
          many.shift(); if(many.length == 0) {
            socket.extensionsInstalled = YES;
            return self.installList(socket, list);
          }
        });
        for(j=0; j<names.length; ++j) {
          var name = names[j],
              ext = exts[namespace][name],
              type = '%@.%@'.f(namespace, ext.type),
              query = "select xt.install_orm('%@')";
          ext = XT.json(this.cleanse(ext)); 
          query = query.f(ext);
          socket.send("installing extension %@".f(type));
          XT.db.query(query, function(e, r) {
            if(e) {
              socket.json.emit('error', { message: e.message,
                context: "installing extension %@".f(etype) });
            }
            process.emit('ORMEXTENSIONINSTALLED');
          });
        }
      }
