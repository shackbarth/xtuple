...
XT.ormInstaller = XT.Object.create(
  ...
  populateSocket: function(socket) {
    ...
    XT.dive(root, { recursive: YES, filter: filter },
      function(err, file) {
        ...
        // care of missing dependencies or errored ones
        try {
          ...
          if(ext) extensions = XT.addProperties(extensions, ns, type, json);
    ...
    var namespaces = XT.keys(extensions),
        namespace, names, name, i, j, ext, type, ns, orm;
    for(i=0; i<namespaces.length; ++i) {
      namespace = extensions[namespaces[i]];
  ...
  */
  dependenciesFor: function(socket, orm, deps) {
    ...
    for(i=0; i<exts.length; ++i) {
      ...
      this.debug("ext => %@.%@".f(ext.nameSpace, ext.type), socket);
  ...
  */
  checkDependencies: function(socket, orm) {
    ...
    this.debug("found ORM %@.%@ to be %@".f(orm.nameSpace, orm.type, enabled ? 'enabled' : 'disabled'), socket);
  ...
  */
  submit: function(socket, name, orm, list, installed, isExtension) {
    socket.send("installing %@ %@".f(isExtension ? '(extension)' : '', name));
    var query = "select xt.install_orm('%@')",
        self = this,
        exts = socket.extensions,
        namespace = orm.nameSpace,
        type = orm.type, text, ext, extName;
      ...
      if(err) {
        socket.json.emit('error', { message: err.message,
          context: "installing %@ %@".f(
            isExtension ? '(extension)' : '', orm.type) });
