  ...
  XM.Model = XM.Model.extend(/** @lends XM.Model# */{
    ...
    */
    ...
    getReportPayload: function (action, options) {
      var modelName = this.reportRecordType || this.editableModel || this.recordType,
        // XXX - fix this crap below to correctly look up userPreferencePrinter
        // ALSO have it also check XM.forms for default printer. Unless ...?
        userPref = _.find(XM.forms.models, function (form) {
          if (form.get("description") === modelName) {
            return form.getValue("defaultPrinter.code");
          }
        }),
        userPrefPrinter = userPref.getValue("defaultPrinter.code");
      return {
