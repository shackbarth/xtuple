...
(function () {
  ...
  */
  ...
  XM.Model = XM.Model.extend(/** @lends XM.Model# */{
    ...
    /**
      Flag whether you want this model to keep track of changes to generate
      JSON patch output. Adds overhead, so only use when needed.
    */
    trackChanges: false,
    ...
    */
    ...
    generateJSONPatch: function () {
      if (!this._mirror) { return []; }
      var obj = this.toJSON(),
        patches = [];
      _generate(this._mirror, obj, patches, "");
      return patches;
    },
  ...
  */
  ...
  /** @private
    Function that generates JSON patch results.
    Thanks to: https://github.com/Starcounter-Jack/JSON-Patch.git
  */
  function _generate(mirror, obj, patches, path) {
    var newKeys = Object.keys(obj),
      oldKeys = Object.keys(mirror),
      changed = false,
      deleted = false,
      key;
