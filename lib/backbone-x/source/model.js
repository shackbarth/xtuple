  ...
  */
  ...
  XM.Model = XM.Model.extend(/** @lends XM.Model# */{
      ...
      for (i = 0; i < relations.length; i++) {
        if (relations[i].type === Backbone.HasMany &&
          ...
          if (!this.isReadOnly()) {
            ...
            this.on('remove:' + relations[i].key, this.relationRemoved);
          }
        }
      }
      this._idx = {};
    ...
    fetch: function (options) {
      ...
      if (klass.canRead()) {
        ...
        options.success = function (resp) {
          ...
          model.lockDidChange(model, model.lock);
          if (XT.debugging) {
    ...
    */
    ...
    /**
      Delete objects marked as destroyed from arrays.
    ...
    toJSON: function (options) {
      ...
      if (includeNested) {
        ...
        _.each(nested, function (rel) {
          rel.options.oldIncludeInJSON = rel.options.includeInJSON;
        ...
        if (relation.type === Backbone.HasMany) {
          ...
          if (attr && attr.length) {
            ...
            for (i = 0; i < attr.models.length; i++) {
              ...
              // If clean the server never knew about it remove entirely
      ...
      if (includeNested) {
        _.each(nested, function (rel) {
          rel.options.includeInJSON = rel.options.oldIncludeInJSON;
    ...
    */
              ...
              if (typeName) {
                if (_.isArray(obj[attr])) {
                  for (i = 0; i < obj[attr].length; i++) {
                    obj[attr][i] = parse(typeName, obj[attr][i]);
                  }
                } else {
                  obj[attr] = parse(typeName, obj[attr]);
                }
              }
            } else {
    ...
    relationAdded: function (model, related, options) {
      ...
      this._idx[type].push(model.id);
    },
    
    relationRemoved: function (model, related, options) {
