    ...
    */
    ...
    getReportUrl: function (action) {
      var modelName = this.reportRecordType || this.editableModel || this.recordType,
        auxilliaryInfo;
      if (this.requiresDetail && this.getValue("detail").length) {
      ...
      return XM.Model.getReportUrl(action, modelName, this.id, auxilliaryInfo);
    },
    ...
    getReportPayload: function (action, options) {
      var modelName = this.reportRecordType || this.editableModel || this.recordType,
        ...
        printQty = this.quantityTransactedAttribute ? this.get(this.quantityTransactedAttribute):
          null;
      /*
      ...
      */
      if (!auxilliaryInfo && (this.requiresDetail && this.getValue("detail").length)) {
        auxilliaryInfo = JSON.stringify({
          detail: _.map(this.getValue("detail").models, function (model) {
            return {
              quantity: model.get("quantity"),
