...
/*global XT:true, XM:true, Backbone:true, _:true, setInterval:true,
  clearInterval:true, jsonpatch:true, Globalize:true, console:true*/
  ...
  */
  ...
  XM.Model = XM.Model.extend(/** @lends XM.Model# */{
    ...
    obtainLock: function (options) {
      ...
      this._reentrantLockAction('obtain', params, options);
    },
    ...
    renewLock: function (options) {
      this._reentrantLockAction('renew', [ this.lock.key ], options);
    },
    ...
    releaseLock: function (options) {
      ...
      if (this.hasLockKey()) {
        this._reentrantLockAction('release', { key: this.getLockKey() }, options);
        this.lockDidChange(this, _(this.lock).omit('key'));
      }
    ...
    */
    bindEvents: function () {
      ...
      _(_(this.relations).where({ type: Backbone.HasMany, includeInJSON: true }))
    ...
    */
     ...
     * Prepare fetch.
     *
     * XXX sync() alone should handle all of this stuff. fetch() is not a
     * Backbone customization point by design.
     */
    _prepareFetch: function (_options) {
      ...
      var options = _.extend({ }, _options),
         ...
         * @callback
         */
        done = _(function (resp) {
          this.setStatus(XM.Model.READY_CLEAN, options);
       ...
       */
      var syncSuccess = _(function (resp) {
        var schema = XT.session.getSchemas()[this.recordType.prefix()],
          lockable = schema.get(this.recordType.suffix()).lockable;

        done = _(done).partial(resp);

        if (lockable && options.obtainLock !== false) {
          this.obtainLock({ success: done });
        }
        else {
          done();
        }
    ...
    */
    fetch: function (_options) {
      var options = this._prepareFetch(_options);
    ...
    */
    ...
    /**
      Returns a formatted value (if available)
      for the property matching the key parameter.
    ...
    */
    getFormattedValue: function (key) {
      ...
      if (value) {
        ...
        // This could possibly be done a bit more gracefully
        } else if (key === "characteristics") {
          var charModel, characteristic,
            charName, charValue, charValues = [];
          for (var i = 0; i < value.models.length; i++) {
            charModel = value.models[i];
            if (charModel) {
              characteristic = charModel.get("characteristic");
              charName = characteristic.get(characteristic.nameAttribute);
              charValue = charModel.get("value");
              if (_.isDate(charValue)) {
                charValue = XT.date.applyTimezoneOffset(charValue, true);
                charValue = Globalize.format(charValue, "d");
              }
              charValues.push(charName +  ": " + charValue);
            }
          }
          value = charValues;
        } else if (value.formatShort) {
          value = value.formatShort();
