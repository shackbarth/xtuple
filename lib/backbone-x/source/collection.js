...
(function () {
  ...
  XM.Collection = Backbone.Collection.extend(/** @lends XM.Collection# */{
    ...
    */
    ...
    //
    //Do we want instances of this collection to bind to the Backbone Relational store
    //and keep themselves in sync with it? New: all caches are auto-synched
    //
    //isAutoSync: false,
    ...
    autoSync: function (model, status) {
      ...
      if (status === XM.Model.BUSY_COMMITTING) {
        this._busyCommitting = true;
      }

      if ((status === XM.Model.READY_CLEAN && this._busyCommitting) ||
          status === XM.Model.DESTROYED_DIRTY) {

        this._busyCommitting = false;
