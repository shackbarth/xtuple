  ...
  XM.ModelClassMixin = {
    ...
    augment: function (hash) {
      ...
      _.each(hash, function (value, key) {
        var existingObj = that.prototype[key];
        if (!existingObj) {
          ...
          // add array elements (for now we allow duplicates)
          that.prototype[key] = _.union(existingObj, value);
        } else if (_.isObject(value)) {
          // mix in objects
          _.extend(existingObj, value);
        } else if (_.isFunction(value)) {
          // for functions, call the super() first, and then the
          // function that's being mixed in
          that.prototype[key] = function () {
