...
(function () {
  ...
  */
  var _canDoAttr = function (action, attribute) {
    ...
    var hasPriv = priv ? XT.session.getPrivileges().get(priv) : true;
      ...
      if (options.collection) {
        ...
        options.collection.each(function (obj) {
          ...
          case "create":
            ...
            if (obj.binaryField) {
              throw "Processing of 'create' for arrays of models with binary fields is not supported.";
            }
            payload.requery = options.requery;
            break;
          case "read":
            throw "Processing 'read' of arrays of models is not supported. Use 'fetch'.";
          case "patch":
          case "update":
            if (obj.binaryField) {
              throw "Processing of 'update' for arrays of models with binary fields is not supported.";
            }
            item.etag = obj.etag;
            item.lock = obj.lock;
      ...
      if (_.isString(value) && value.indexOf('.') !== -1) {
        ...
        _.each(parts, function (part) {
          ...
          if (i < parts.length) {
            ...
            result = XT.getObjectByName(relation.relatedModel);
          } else {
