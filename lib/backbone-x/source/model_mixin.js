  ...
  XM.ModelMixin = {
    ...
    */
    ...
    generatePatches: function () {
      ...
      var obj = this.toJSON(),
        observer = XM.jsonpatch.observe(this._cache);

      // 1) change the object being observed on the "observer"
      // 2) deal with the internal problem on json patch where
      // the date is converted to string by the parse/stringify
      // process. We do that here too to ensure date comparisons
      // are apples to apples.
      observer.object = JSON.parse(JSON.stringify(obj));
    ...
    dispatch: function (name, func, params, options) {
      ...
      var dataSource = options.dataSource || XT.dataSource,
        payload = {
          ...
          databaseType: options.databaseType || this.databaseType
        };
    ...
    */
    parse: function (resp) {
      var K = XT.Session,
        schema = XT.session.getSchema(),
        column,
        parse;
      parse = function (typeName, obj) {
      ...
      return parse(this.recordType.suffix(), resp);
    },
    ...
    */
    sync: function (method, model, options) {
      ...
      payload.databaseType = options.databaseType || this.databaseType;
      ...
      if (options.id) {
        ...
        payload.id = this._cache[this.idAttribute];
      } else if (this.id) {
        payload.id = this.id;
      } else if (this.attributes) {
        payload.id = this.attributes[this.idAttribute];
      } else {
        return;
      }
