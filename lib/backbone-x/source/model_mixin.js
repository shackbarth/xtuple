  ...
  */
  var _canDoAttr = function (action, attribute) {
    var priv = this.privileges &&
      ...
      this.privileges.attribute[action] &&
      _.contains(this.privileges.attribute[action]properties, "attribute" ?
      this.privileges.attribute[action]privilege : false;
    ...
    */
    validate: function (attributes, options) {
      ...
      if (!XT.session.getSchemas()[this.recordType.prefix()].get(this.recordType.suffix())) {
        console.log("Cannot find schema", this.recordType);
      }
      var i,
        S = XT.Session,
  ...
  XM.ModelClassMixin = {
    ...
    */
    canUpdate: function (model) {
      return XM.ModelClassMixin.canDo.call(this, 'update', model, attribute);
    },
