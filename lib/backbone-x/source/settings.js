  ...
  */
  XM.Settings = XM.Model.extend(/** @lends XM.Settings# */{
    ...
    fetch: function (options) {
      var prepareOptions = _.extend({ }, options),
        userCallback = options.success,
        done = function (resp, options) {
          if (!this.set(this.parse(resp, options), options)) {
            return false;
          }
          if (_.isFunction(userCallback)) {
            userCallback(this, resp, options);
          }
        },
        fetchOptions = this._prepareFetch(_(prepareOptions).extend({
          success: done
        }));

      return fetchOptions && XM.Model.prototype.fetch.call(this, fetchOptions);
    },
