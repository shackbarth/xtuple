  ...
  X.Server.create({
    ...
    remap: function (req, res, next) {
      ...
      if (!url.match(/client/g) && !url.match(/login/g)
          && !url.match(/public-extensions/g) && !url.match(/private-extensions/g)) {
    ...
    route: function (req, res, next) {
      ...
      (xtr = X.Response.create({
        ...
        var routers = this.get("routers"),
          router, routes, path = xtr.get("path"), i = 0, regexRoute;

        for (; i < routers.length; ++i) {
          router = routers[i];
          routes = router.routes;

          // Check for a matching regexRoute and use that.
          ...
          // Check for a matching full route and use that.
          if (routes[path]) {
            router.route.call(router, xtr);
            return;
          }
