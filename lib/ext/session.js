...
(function () {
  ...
  X.Session = X.Object.extend(/** @lends X.Session.prototype */{
    ...
    query: function (query, callback) {
      ...
      if (payload.databaseType === 'global') {
        // TODO XXX Authentication!?!!?
        ...
        if (payload.requestType === 'fetch') {
          ...
          //model = new XM[payload.recordType.substring(3)](payload.dataHash);
          var fetchObj = {}, Model;
          // XXX can't get this to work. Solution loosely modeled on
          // http://whaticode.com/2012/09/13/backbone-relational-and-duplicate-objects/
          // with lots of variations tried
          // without this convolution BR complains when a business object is saved a second time
          // with the convolution it breaks
          Model = XM[payload.recordType.substring(3)];
          fetchObj[Model.prototype.idAttribute] = payload.dataHash[Model.prototype.idAttribute];
          model = Model.findOrCreate(fetchObj);
          model.fetch(fetchObj, {
            success: function (model, resp) {
              model.set(payload.dataHash);
              options.force = true;
              XT.dataSource.commitRecord(model, options);
            },
            error: function () {
              X.log("Error fetching for commit");
            }
          });
        }
      } else {
