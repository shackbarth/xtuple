...
(function () {
  ...
  X.Session = X.Object.extend(/** @lends X.Session.prototype */{
    ...
    commit: function () {
      ...
      if (this.get("isLoaded")) {
        ...
        options.error = function (err) { that.updateForCommit(err); };
    ...
    updateForCommit: function (err, res) {
      ...
      options.success = function () { that.didCommit(); };
      options.error = function (err) { that.didCommit(err); };
      res.save(options);
    },
    ...
    didLoadForSelection: function (err, res) {
      ...
      if ((found = _.find(organizations.models, function (o) {
        return o.get('name') === selected ? true: false;
      }))) {
        ...
        this.set("username", found.get('username'));
  ...
  X.mixin(X.Session, /** @lends X.Session */{
    ...
    load: function (callback) {
      ...
      options.error = function (err) { callback(err); };
