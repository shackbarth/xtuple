  ...
  X.Session = X.Object.extend(/** @lends X.Session.prototype */{
    ...
    query: function (query, callback) {
      ...
      if (payload.databaseType === 'global') {
        // Gangnam style authentication
        var authorizedUsers = ["admin", "jrogelstad", "shackbarth"];
        if (authorizedUsers.indexOf(this.username) < 0 &&
            (payload.requestType !== 'dispatch' || payload.functionName !== 'schema')) {
          //this.set("error", "unauthorized access");
          // not sure why that crashes the dataserver
          return;
        }
        ...
        if (payload.requestType === 'fetch') {
          ...
          if (!payload.dataHash) {
            return this.set("error", "invalid commit");
          }
          id = payload.dataHash[Model.prototype.idAttribute];
          model = Model.findOrCreate(id);
          if (model === null) {
            // this bit should not be necessary by my understanding of findOrCreate. Go figure.
            model = new Model();
          }
          model.set(payload.dataHash, {force: true});
