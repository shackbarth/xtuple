...
// PostgreSQL error: Error: operator does not exist: bigint = date
          ...
          { attribute: "username", value: this.get("username") || -1 }
        ]
      };

      validateOptions.username = this.get("username");

      validateOptions.success = function (resp) {
        var options = {};

        options.id = that.get("sid") || -1;
        options.success = function (resp) { callback(null, resp); };
        options.error = function (model, err) { callback(err); };

        k.fetch(options);
      };
      validateOptions.error = function (model, err) {
        callback(err);
      };
