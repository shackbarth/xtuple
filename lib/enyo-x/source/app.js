...
(function () {
  ...
  enyo.kind({
    ...
    /*
      ...
      Currently, the things that are updated are the caches and the lists.
    */
    modelChanged: function (inSender, inEvent) {
      var modelName = inEvent.model,
        model,
        modelId = inEvent.id,
        cacheName = modelName.suffix().decapitalize();

      //
      // update the cache if we have one
      //
      if (XM[cacheName]) {
        model = _.find(XM[cacheName].models, function (model) {
