...
(function () {
  ...
  enyo.kind(
    ...
    handleKeyDown: function (inSender, inEvent) {
      ...
      var magstripeStart = [16, 53, 16, 66];
      var magstripeEnd = [191, 13];
      if (_.isEqual(this._keyBufferArray.slice(-1 * magstripeStart.length), magstripeStart)) {
        this._keyBuffer = this._keyBuffer || "";
        this._keyBufferEndPattern = magstripeEnd;
      }

      if (this._keyBufferEndPattern && _.isEqual(this._keyBufferArray.slice(-1 * this._keyBufferEndPattern.length), this._keyBufferEndPattern)) {
        console.log("here it is", this._keyBuffer);
        this._keyBuffer = "";
