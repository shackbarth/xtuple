...
(function () {
 ...
 /**
  ...
  XV.EditorMixin = {
    controlValueChanged: function (inSender, inEvent) {
      var key = inEvent.originator.attr,
        value = inEvent.value;

      if (this.value) {
        this.value.set(_.isObject(key) ? value : _.object([key], [value]));
      }
      return true;
    },
    /**
  ...
  */
  var _setBindings = function (ws, action) {
    ...
    if (ws.value.meta) {
      ws.value.meta[action]("change", ws.attributesChanged, ws);
    }
  };
  ...
  */
  enyo.kind(_.extend({ },
    ...
    // TODO: Causing some bugs, so activate with widget refactor
    /**
     * If a Workspace has a single Panels component, delete it and attach
     * its children directly to the Workspace.
     * XXX #refactor out this hack
    */
    // initComponents: function () {
    //   var kindComponents = XV[this.kind.suffix()].prototype.kindComponents.slice(0),
    //     head = [ ], tail = [ ];
    //
    //   if (kindComponents[0].kind.indexOf('Panels') !== -1) {
    //     head = head.concat(kindComponents[0].components);
    //     tail = tail.concat(_.tail(kindComponents));
    //   }
    //   head = head.concat(this.processExtensions());
    //   this.kindComponents = head.concat(tail);
    //
    //   this.inherited(arguments);
    // },
