...
(function () {
  ...
  */
  enyo.kind(_.extend({ },
    ...
    */
    print: function () {
      var printer = XT.defaultPrinter(this.model);
      if (printer) {
        var printOptions = {
          printer: printer
        };
        // send it to be printed silently by the server
        this.getValue().doPrint(printOptions);
      } else {
        this.openReport(XT.getOrganizationPath() + this.getValue().getReportUrl());
      }
    },
  ...
  enyo.kind(/** @lends XV.WorkspaceContainer# */_.extend({
    ...
    save: function (options) {
      ...
      // Should be "OR" right? The printOnSaveSetting set on workspace OR set in metrics...
