          ...
          if (_.isObject(attribute)) {
            ...
            for (var attr in obj) {
              if (obj.hasOwnProperty(attr)) {
                ...
                // to be modified
                isReadOnly = isReadOnly || model.isReadOnly(modelPropName);
  ...
  */
  ...
  var workspaceHash = enyo.mixin(XV.EditorMixin, /** @lends XV.Workspace# */{
    ...
    recordIdChanged: function () {
      ...
      // we can't assume the idAttribute is "id"!
      findOrCreateObj[Klass.prototype.idAttribute] = recordId;
      this.value = Klass.findOrCreate(findOrCreateObj);
      _setBindings(this, 'on');
      this.value.fetch();
    },
    /**
  ...
  enyo.kind(/** @lends XV.WorkspaceContainer# */{
    ...
    print: function (inSender, inEvent) {
      ...
      window.open('/report?details={"nameSpace":"%@","type":"%@","id":%@,"locale":%@}'
        // TODO: use prefix()
        .f(model.recordType.substring(0, model.recordType.indexOf('.')),
          model.recordType.suffix(), model.id, JSON.stringify(XT.session.locale.toJSON())), '_newtab');
    },
    /**
    ...
    save: function (options) {
      ...
      if (this.$.workspace.printOnSaveSetting &&
