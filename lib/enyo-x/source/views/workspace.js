    ...
    */
    attributesChanged: function (model, options) {
      ...
      _.each(ctrls, function (control) {
        attributes = this.findAttributes(control);
        if (attributes) {
          _.each(attributes, function (attr) {
            if (changes.hasOwnProperty(attr)) {
              ...
              if (control.setValue && !(status & K.BUSY)) {
                control.setValue(value, {silent: true}, attr);
              }
          ...
          // TODO: if this control doesn't have an attribute, it might be calculated
        }
      }, this);
    ...
    /**
     Returns the attribute string or object contained in the control.
     */
    findAttributes: function (ctl) {
      var values = [], attrs = this.value ? this.value.getAttributeNames() : [];
      values = _.filter(attrs, function (attr) {
        // ctl.attr may be an object
        if (_.isObject(ctl.attr)) {
          return _.filter(_.values(ctl.attr), function (str) {
            return str === attr;
          });
        }
        return ctl.attr === attr;
