...
(function () {
  ...
  */
   ...
   */
  var workspaceHash = enyo.mixin(/** @lends XV.Workspace# */{
    name: "XV.Workspace",
    kind: "FittableRows",
    modelView: "EnyoView",
    published: {
  ...
  enyo.kind(/** @lends XV.WorkspaceContainer# */{
    ...
    components: [
      ...
      {kind: "FittableRows", name: "contentPanel", components: [
        {kind: "onyx.MoreToolbar", name: "contentToolbar", components: [
          ...
          {kind: "onyx.Button", name: "reportButton", showing: false,
            content: "_report".loc(), onclick: "report"},
    ...
    /**
     Calls the report route to show a report of the model's data
     ...
     */
    reportNew: function () {
      window.open(XT.getOrganizationPath() + this.$.workspace.value.getReportUrl(),
        "_newtab");
    },
    /**
     Calls the report route to print the model's data
     */
    printNew: function (kind, id) {
      this.$.workspace.getValue().doPrint();
    },
    /**
     Calls the report route to show a report of the model's data
     */
    report: function () {
      if (this.$.workspace.getValue().recordType === 'XM.Invoice') {
        this.reportNew();
        return;
      }
      var model = this.$.workspace.getValue(),
        reportModelName = this.$.workspace.getReportModel() || model.recordType,
        reportName = this.$.workspace.getReportName() || reportModelName.suffix(),
        details = {
          nameSpace: reportModelName.prefix(),
      ...
      window.open(XT.getOrganizationPath() +
        "/report?details=%@".f(JSON.stringify(details)), "_newtab");
    },
    /**
     Calls the report route to print the model's data
     */
    print: function (inSender, inEvent) {
      ...
      if (this.$.workspace.getValue().recordType === 'XM.ShipShipment') {
        XM.Model.getReportUrl('print', 'XM.Invoice', inEvent.invoiceNumber);
        return;
      }
      var model = this.$.workspace.getValue(),
        reportModelName = this.$.workspace.getReportModel() || model.recordType,
      ...
      XT.DataSource.callRoute(
        ...
        {success: function (result) {
          ...
          if (XT.session.config.debugging) {
            XT.log("Print route success, type: ", reportModelName.suffix());
          }
        },
        error: function (result) {
          if (XT.session.config.debugging) {
            XT.log("Print route error, type: ", reportModelName.suffix());
          }
        }
        }
      );
