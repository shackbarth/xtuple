  ...
  */
  ...
  var workspaceHash = enyo.mixin(XV.EditorMixin, /** @lends XV.Workspace# */{
    ...
    recordIdChanged: function () {
      ...
      this.value = Klass.findOrCreate({id: recordId});
  ...
  enyo.kind(/** @lends XV.WorkspaceContainer# */{
    ...
    components: [
      ...
      {kind: "FittableRows", name: "contentPanel", components: [
        ...
        {kind: "onyx.Popup", name: "errorPopup", centered: true,
          ...
          {kind: "onyx.Button", content: "_cancel".loc(), name: "errorCancel", ontap: "errorOk",
            classes: "xv-popup-button", showing: false}
        ]},
    ...
    notify: function (inSender, inEvent) {
      ...
      typeToButtonMap[String(XM.Model.YES_NO_CANCEL)] = ["errorYes", "errorNo", "errorCancel"];
    ...
    print: function (inSender, inEvent) {
      ...
      window.open('/report?details={"recordType":"%@","id":%@,"locale":%@}'
        .f(model.recordType, model.id, JSON.stringify(XT.session.locale.toJSON())), '_newtab');
    },
    /**
