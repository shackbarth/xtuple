  ...
  */
  var _setBindings = function (ws, action) {
    ...
    if (headerAttrs.length) {
      for (i = 0; i < headerAttrs.length; i++) {
        attr = headerAttrs[i];
        if (attr.indexOf('.') !== -1 ||
            _.contains(ws.value.getAttributeNames(), attr)) {
  ...
  var workspaceHash = enyo.mixin(/** @lends XV.Workspace# */{
    ...
    buildMenu: function () {
      ...
      var actionMenu = this.container.container.$.actionMenu,
      ...
      actionMenu.render();
      this.container.container.$.actionButton.setShowing(actions.length);
    },
    /**
  ...
  enyo.kind(/** @lends XV.WorkspaceContainer# */{
    ...
    actionSelected: function (inSender, inEvent) {
      ...
      this.$.workspace[method](inSender, inEvent);
    },
    ...
    */
    setWorkspace: function (options) {
      ...
      if (workspace) {
        ...
        if (workspace.getHideActionButton()) {
