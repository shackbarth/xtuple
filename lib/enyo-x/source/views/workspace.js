...
(function () {
  ...
  */
  enyo.kind(_.extend({ },
    ...
    */
    print: function () {
      var printer = XT.defaultPrinter(this.model.suffix());
      if (printer) {
        // send it to be printed silently by the server
        this.getValue().doPrint({printer: printer});
      } else {
        this.openReport(XT.getOrganizationPath() + this.getValue().getReportUrl());
      }
    },
  ...
  enyo.kind(/** @lends XV.WorkspaceContainer# */_.extend({
    ...
    save: function (options) {
      ...
      /* Some workspaces are set up with a setting to have them auto print when they're saved.
        The printOnSaveSetting set on workspace *OR* set in metrics. TODO - this should be moved
        to workspace.save, within the success function so that the print call is not called if user
        backs/errors out of save.
      */
      if (workspace.printOnSaveSetting || XT.session.settings.get(workspace.printOnSaveSetting)) {
