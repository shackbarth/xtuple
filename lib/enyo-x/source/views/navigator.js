...
(function () {
  ...
  var navigator = /** @lends XV.Navigator# */{
    ...
    */
    ...
    /**
      When columns are selected in the layout panel,
      they are either removed from their current location or
      added to a different slot (addition of row if necessary)
    */
    changeLayout: function (inSender, inEvent) {
      var attr = inEvent.attr,
        showing = inEvent.show,
        list = this.$.contentPanels.getActive(),
        currentColumns = list.getCurrentListAttributes(),
        offColumn;
    },
    ...
    */
    ...
    showParameters: function (inSender, inEvent) {
      ...
      if (parameterWidget && parameterWidget.showLayout) {
        parameterWidget.buildColumnList(list.getDisplayAttributes(), list.getCurrentListAttributes());
      }
    },
    /**
