...
(function () {
  ...
  var navigator = /** @lends XV.Navigator# */{
    ...
    */
    ...
    /**
      When a new column value is selected in the layout panel, this
      new value is removed from any column where it current exists and
      it then replaces the old attribute in its location.
    */
    changeLayout: function (inSender, inEvent) {
      var oldAttr = inEvent.attr,
        newValue = inEvent.value,
      ...
      // remove the new value from any columns where
      // it might already exist, unless it is empty
      if (newValue) {
        _.each(currentColumns, function (col) {
          if (col.attr === newValue) {
            // if we find the new value, remove it
            col.setAttr("");
          }
        });
      }
      // find the old attribute value and replace it
      // with the new value
      _.each(currentColumns, function (col) {
        if (col.attr === oldAttr) {
          // if we find the new value, remove it
    ...
    */
    ...
    showParameters: function (inSender, inEvent) {
      ...
      // If there is a parameter widget, set the columns for layout
      this.buildLayout();
    },
    /**
