  ...
  enyo.kind(/** @lends XV.Navigator# */{
    ...
    */
    ...
    listActionSelected: function (inSender, inEvent) {
      ...
      // TODO: fold in delete with the rest if we can
      if (originator.name === 'delete') {
        notifyEvent.message = "_confirmDelete".loc() + " " + "_confirmAction".loc();
        notifyEvent.callback = function (answer) {
          if (answer) {
        ...
        if (originator.action.notify) {
          notifyEvent.message = originator.action.notifyMessage || "_confirmAction".loc();
          notifyEvent.callback = function (answer) {
            if (answer) {
              that.listActionConfirmed(that._listItemEvent, originator.action);
            }
          };
          this.doNotify(notifyEvent);
        } else {
          that.listActionConfirmed(this._listItemEvent, originator.action);
        }
      }
      return true;
    },
    listActionConfirmed: function (event, action) {
      this._listItemEvent.originator.parent.doActionSelected({model: event.model, action: action});
    },
    ...
    */
    ...
    showListItemMenu: function (inSender, inEvent) {
      ...
      // then add any list-specific actions if applicable
      _.each(inEvent.actions, function (action) {
        ...
        if (component) {
          // if it's already been created, just show it
          component.show();
        } else {
          // otherwise make it
          that.$.listItemMenu.createComponent({
            name: action.name,
