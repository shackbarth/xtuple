...
(function () {
  ...
  enyo.kind(/** @lends XV.Navigator# */{
    ...
    */
    ...
    /**
      Routes the action to the appropriate context: the enyo kind or the model
     */
    listActionConfirmed: function (event, action) {
      ...
      } else if (model instanceof XM.Info) {
        XT.getObjectByName(model.editableModel)[action.method](model);

      } else {
        event.model[action.method](); // TODO: some arguments may one day be necessary in here
      }
    },
    ...
    */
    ...
    showListItemMenu: function (inSender, inEvent) {
      ...
      _.each(inEvent.actions, function (action) {
        ...
        if (component && !inEvent.actionPermissions[action.name]) {
          // if it's already been created but we don't have permissions, hide it
          component.hide();

        } else if (component) {
          // if it's already been created and we have permissions, just show it
          component.show();

        } else if (inEvent.actionPermissions[action.name]) {
