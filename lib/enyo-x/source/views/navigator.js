  ...
  enyo.kind(/** @lends XV.Navigator# */{
    ...
    */
    ...
    listActionSelected: function (inSender, inEvent) {
      ...
      case 'delete':
        newEvent.message = "_confirmDelete".loc() + " " + "_confirmAction".loc();
        newEvent.callback = function (answer) {
          if (answer) {
            that._listItemEvent.originator.parent.doDeleteItem(that._listItemEvent);
          }
        };
        break;
      // XXX it would be easy enough to generalize this event (e.g. doItemAction)
      // and just make the list know what function to map it to.
      case 'convert':
        newEvent.message = "_confirmAction".loc();
        newEvent.callback = function (answer) {
          if (answer) {
            that._listItemEvent.originator.parent.doConvertItem(that._listItemEvent);
          }
        };
        break;
      }
      this.doNotify(newEvent);
      return true;
    },
    printList: function (inSender, inEvent) {
      ...
      window.open('/report?details={"nameSpace":"%@","type":"%@","query":%@,"locale":%@}'
        .f(recordType.substring(0, recordType.indexOf('.')), // TODO: use prefix
          recordType.suffix(),
    ...
    */
    ...
    setupModuleMenuItem: function (inSender, inEvent) {
      ...
      if (index) { // make this isn't an airb
    ...
    panelTap: function (inSender, inEvent) {
      var index = inEvent.index;
      if (index && inSender.isSelected(index)) { // make sure an item in the list was clicked and is selected
        this.setContentPanel(index);
      }
    },
    menuTap: function (inSender, inEvent) {
      if (inEvent.index) { // make sure an item in the list was clicked
        this.setupModuleMenuItem(inSender, inEvent);
      }
    },
    /**
      Error notification, now re-routed to XV.ModuleContainer notify mechanism
     */
    ...
    showListItemMenu: function (inSender, inEvent) {
      ...
      _.each(this.$.listItemMenu.$, function (value, key) {
        if (value.kind === 'onyx.MenuItem') {
          value.hide();
        }
      });

      // then add the delete menu option if applicable
      var deleteComponent = this.$.listItemMenu.$.delete;
      if (deleteComponent && !inEvent.allowDelete) {
        deleteComponent.hide();
      } else if (!deleteComponent && inEvent.allowDelete) {
        that.$.listItemMenu.createComponent({name: "delete", content: "_delete".loc()});
      } else if (inEvent.allowDelete) {
        deleteComponent.show();
      }

      // then add any list-specific gear actions if applicable
      _.each(inEvent.gearActions, function (action) {
        var component = _.find(that.$.listItemMenu.$, function (value, key) {
          return key === action;
        });
        if (component) {
          component.show();
        } else {
          that.$.listItemMenu.createComponent({name: action, content: ("_" + action).loc()});
        }
      });
      this.$.listItemMenu.render();

      // the rest of the convoluted code to convince the Menu that it lives in the list
      this.$.listItemMenu.requestMenuShow(this, {activator: inEvent.originator});
      this.$.listItemMenu.adjustPosition();
    },
    /**
