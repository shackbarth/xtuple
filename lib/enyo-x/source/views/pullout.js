      ...
      {name: "containerPanels", kind: "enyo.Panels", style: "height: 100%", draggable: false, components: [
        {name: "history", components: [
          ...
          {kind: "Scroller", components: [
            // TODO: this should probably be an enyo.List
        ...
        {name: "help", components: [
          {classes: "xv-pullout-header", content: "_help".loc()},
          {tag: "iframe", name: "helpPage", attributes: {height: "90%"}}
        ]}
      ]}
    ],
    /**
    ...
    */
    togglePullout: function (inSender, inEvent) {
      ...
      var name = inEvent.name,
        ...
        // selectedPanel === name if the user is closing the pullout with an icon button
          ...
          if (panelExists) {
            ...
            var panel = this.$.containerPanels.createComponent(
              {kind: "Scroller", name: item.name + "Scroller", components: [
                {classes: "xv-pullout-header", content: "".loc()}, // set content for header
                item
              ]}
            );
            panel.render();
      ...
      if (forceMin && !isGripperOriginated) {
        //
