  ...
  /** @lends XV.GridBox# */{
    ...
    */
    ...
    gridRowTapEither: function (index, indexStart) {
      ...
      this.setEditableIndex(index + indexStart);
      this.$.aboveGridList.setCount(this.getEditableIndex());
      // XXX hack. not sure why the port doesn't know to resize down
      this.$.aboveGridList.$.port.applyStyle("height", (101 * this.getEditableIndex()) + "px");
      this.$.aboveGridList.render();
      this.$.editableGridRow.setValue(this.getValue().at(this.getEditableIndex()));
      this.$.editableGridRow.show();
      this.$.belowGridList.setCount(this.getValue().length - this.getEditableIndex() - 1);
    ...
    */
    ...
    setupRowEither: function (index, gridRow, indexStart) {
      ...
      if (model.isDestroyed()) {
        gridRow.hide();
        gridRow.applyStyle("height", "0px");
      } else {
        gridRow.show();
        gridRow.applyStyle("height", "100px");
