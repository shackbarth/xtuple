...
// TODO: picker onblur
  ...
  */
  ...
  /** @lends XV.GridBox# */{
    ...
    */
    gridRowTap: function (inSender, inEvent) {
      if (!inEvent.index) {
        // tap somewhere other than a row item
        return;
      }
      this.setEditableIndex(inEvent.index);
      this.$.editableGridRow.setValue(this.getValue().at(inEvent.index));
    ...
    */
    newItem: function () {
      ...
      this.setEditableIndex(this.getValue().length - 1);
      this.valueChanged();
    ...
    setupRow: function (inSender, inEvent) {
      ...
      _.each(this.$.gridRow.$, function (control) {
      ...
      if (model.isDestroyed() || inEvent.index === this.getEditableIndex()) {
        this.$.gridRow.hide();
      } else {
        this.$.gridRow.show();
        ...
        // the superkind.
        this.$.gridRow.$.addGridRowButton.setDisabled(model.isReadOnly());
        this.$.gridRow.$.expandGridRowButton.setDisabled(model.isReadOnly());
        this.$.gridRow.$.deleteGridRowButton.setDisabled(model.isReadOnly());
      }
      return true;
    },
    valueChanged: function () {
