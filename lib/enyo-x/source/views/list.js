        ...
        if (col.kind === "XV.ListColumn") {
          columns.push(
            {kind: "XV.ListColumn", classes: col.classes, components: [
              {kind: "XV.ListAttr", allowLayout: true}
            ]}
          );
        }
      });
    ...
    setupItem: function (inSender, inEvent) {
      ...
      for (prop in this.$) {
        if (this.$.hasOwnProperty(prop)) {
          ...
          if (obj.overflow) {
            var item, showOverFlow;
            for (item in obj.$) {
              if (this.$.hasOwnProperty(item) && !_.isEmpty(item.attr)) {
                showOverFlow = true;
              }
            }
            showOverFlow = false;
            obj.setShowing(showOverFlow);
          }
          if (obj.getAttr) {
            view = obj;
            isPlaceholder = false;
            attr = obj.getAttr();
            if (attr) { // the attribute may be empty if layout is changed
