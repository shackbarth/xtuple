  ...
  enyo.kind(/** @lends XV.List# */{
    ...
    */
    ...
    create: function () {
      ...
      if (!_.find(this.getActions(), function (action) { return action.name === 'delete' })) {
    ...
    */
    ...
    setupItem: function (inSender, inEvent) {
      ...
      if (toggleSelected) {
        ...
        // to give the user a delete menu option if they click the action icon button.
        if (!actionIconButton) {
          console.log("no action icon. this should never happen");
          ...
          this._actionPermissions = {}; // clean slate in case we've changed list items
          _.each(this.getActions(), function (action) {
            console.log(action);
            if (!action.prerequisite) {
              that._actionPermissions[action.name] = true;
              console.log(model.id, action.name, action.prerequisite, "always");
              return;
            }

            if (model instanceof XM.Info) {
              XT.getObjectByName(model.editableModel)[action.prerequisite](model, function (response) {
                that._actionPermissions[action.name] = response;
              });
            } else {
              model[action.prerequisite](function (response) {
                that._actionPermissions[action.name] = response;
              });
            }
          });
    ...
    transformListAction: function (inSender, inEvent) {
      var that = this,
        index = inEvent.index,
        model = this.getValue().models[index],
        haveAllAnswers = _.reduce(this.actions, function (memo, action) {
          return memo && _.isBoolean(that._actionPermissions[action.name]);
        }, true);
        canDoAnything = _.reduce(this.actions, function (memo, action) {
          return memo || that._actionPermissions[action.name];
        }, false);

      if (!canDoAnything) {
        // don't bother bubbling further if there are no allowable actions
        return true;
      }

      if (!haveAllAnswers) {
        console.log("I don't know yet which of the actions you have permissions to do");
        // TODO: implement this via a greyed-out gear
        return true;
      }
