  ...
  enyo.kind(_.extend(/** @lends XV.List# */{
    ...
    */
    ...
    /**
      ...
      another tab. TODO - can't we always just use options.model.recordType?
    */
    doPrint: function (options) {
      var modelName = options.model.modelName || options.model.editableModel,
        printer = XT.defaultPrinter(modelName);
      if (printer) {
        // send it to be printed silently by the server
        options.model.doPrint({printer: printer});
      } else {
        // This is not working when printing multiple reports
        this.openReport(XT.getOrganizationPath() + options.model.getReportUrl());
      }
    },
    /**
