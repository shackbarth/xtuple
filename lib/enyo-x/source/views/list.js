  ...
  enyo.kind(_.extend(/** @lends XV.List# */{
    ...
    */
    ...
    /**
      ...
      Otherwise, show the report in another tab. On new print actions, "formType" key is defined.
      On older print actions we can get the "formType" from the editableModel, i.e. XM.Invoice.
    */
    doPrint: function (options) {
      var formType = (options.action ? options.action.formType : null) || (options.model ? options.model.editableModel : null),
        printer = formType ? XT.defaultPrinter(formType.suffix()) : null;
      if (printer) { // send it to be printed silently by the server
        options.model.doPrint({printer: printer});
