...
(function () {
  ...
  enyo.kind({
    ...
    setPickerStrings: function (inSender, inEvent) {
      ...
      if (this.list.getModel(0))
        attrsToCompare = this.list.getModel(0).getAttributeNames();

      if (XT.getObjectByName(activePanel.collection).prototype.model.prototype.editableModel)
        baseModel = XT.getObjectByName(activePanel.collection).prototype.model.prototype.editableModel;
      else
        baseModel = XT.getObjectByName(activePanel.collection).prototype.model.prototype.recordType; 
      ...
      _.each(this.list.$, function(enyoObj) {
        if (enyoObj.attr) {
          ...
          if (enyoObj.attr.indexOf('.') !== -1) {
            ...
            //if the attr starts with get, it's a function and should be ignored
            if (enyoObj.attr.indexOf("get") !== 0) {
              attrs.push(enyoObj.attr)
            }
          }
        }
