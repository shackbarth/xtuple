  ...
  enyo.kind({
    ...
    */
    ...
    /**
      ...
      values (attr1, attr2, attr3, ...) that are set into corresponding
      pickers.
    */
    ...
    */
    setPickerStrings: function (inSender, inEvent) {
      ...
      allAttributes = this.list.getSortableAttributes();

      // filter the list of attributes to only inlude those viewable in the current list
      _.each(this.list.$, function (enyoObj) {
        if (enyoObj.attr && !_.isFunction(enyoObj.attr)) {
          if (enyoObj.attr.indexOf('.') !== -1) {
            var sliced = enyoObj.attr.split('.');
            if (allAttributes.indexOf(sliced[sliced.length - 1]) !== -1) {
