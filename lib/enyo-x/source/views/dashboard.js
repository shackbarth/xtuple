...
(function () {
  ...
  */
    ...
    */
    ...
    processData: function () {
      ...
      groupedData = _.groupBy(filteredData, function (datum) {
        if (!that.getGroupByField() || !datum[that.getGroupByField()]) {
          return null;
  ...
  enyo.kind({
    ...
    plot: function () {
      var that = this,
        ...
        chartData = [{
          values: _.map(this.getProcessedData(), function (datum) {
            return {
  ...
  enyo.kind({
    ...
    kind: "XV.SelectableChart",
    aggregateData: function (groupedData) {
      // TODO: I think this actually needs to be a histogram of some sort

      var aggregatedData = _.map(groupedData, function (group, key) {
        var groupValues = _.map(group, function (modelData) {
          console.log(modelData);
          return {
            x: modelData.shipDate.getTime(),
            y: modelData.totalPrice
          };
        });
