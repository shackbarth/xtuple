      ...
      var that = this,
        aggregatedData = _.map(groupedData, function (datum, key) {
          var reduction = _.reduce(datum, function (memo, row) {
            ...
            var increment = that.getTotalField() ? row[that.getTotalField()] : 1;
            return {
      ...
      var that = this,
        ...
        dataPoints = _.reduce(groupedData, function (memo, group) {
          _.each(group, function (item) {
            var dateInt = item.get("shipDate").getTime();
            console.log(dateInt);
...
/*
  ...
  */
    ...
    plot: function () {
      ...
      console.log(JSON.stringify(this.getProcessedData()));
