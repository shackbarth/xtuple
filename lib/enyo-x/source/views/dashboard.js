  ...
  */
  enyo.kind({
    ...
    components: [
      ...
      {name: "panels", kind: "ChartsPanels", onTransitionFinish: "transitionFinished"}
    ],
    /**
    ...
    */
    addChart: function (value) {
      var p = this.$.panels,
        ...
        component = {kind: kind, model: value, removeIconShowing: true},
        newChart = p.createComponent(component);
      newChart.setBindings();
      newChart.render();
      p.reflow();
      p.render();
    },
    /**
      Add a new model for the new chart and add it to list
    */
    addModel: function (kind) {
    ...
    /**
      When a menu item is selected, it adds a model
      for the new chart.
    */
    itemSelected: function (inSender, inEvent) {
      if (inEvent.originator.content) {
        this.addModel(inEvent.originator.chart);
      }
    },
    /**
    ...
    */
    ...
    /**
     Save the collection of charts and their picker values.
     */
    saveDashboard: function (inSender, inEvent) {
      var options = {}, that = this,
        K = XM.Model;
      options.success = function (model, resp, options) {
        // We have a save! Great! Now, what?
        console.log("there was just a dashboard save!")
      };

      _.each(this.getValue().models, function (model) {
        // don't bother the model for a save if we don't need it
        if (model.canUpdate && (model.getStatus() === K.READY_NEW || model.isDirty())) {
    ...
    */
    statusChanged: function (inSender, inEvent) {
      var K = XM.Model, canSave = false, message;

      // loop through the models and determine if any of
      // the models can be saved.
      _.each(this.getValue().models, function (model) {
    ...
    */
    transitionFinished: function (inSender, inEvent) {
      // loop through the panels and set their order
      // value based on the arrangement
      // TODO: ***This isn't quite working as expected
      var p = inEvent.originator.getPanels();
  ...
  */
  enyo.kind(
    ...
    */
    ...
    /**
      Set the filter values from the model
    */
