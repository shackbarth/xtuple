  ...
  var transactionListContainer =  /** @lends XV.TransactionListContainer# */ {
    ...
    */
    ...
    print: function () {
      var printableModels,
        that = this;
      if (this.$.list.value.models) {
        printableModels = _.filter(this.$.list.value.models, function (model) {
          return model.get(model.quantityTransactedAttribute) > 0;
        });
        
        if (printableModels) {
          _.each(printableModels, function (model) {
            model.modelName = model.recordType;
            that.$.list.doPrint({model: model});
          });
        }  
      }
    },
