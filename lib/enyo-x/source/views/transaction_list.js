...
(function () {
  ...
  enyo.kind(
    ...
    print: function () {
      var modelName = this._printModels[0].modelName,
        printer = XT.defaultPrinter(modelName);

      _.each(this._printModels, function (printModel) {
        if (printer) {
          printModel.doPrint({printer: printer});
        } else {
      ...
      callback = function (workspace, transFunction, transWorkspace, transQty) {
        ...
        // If a workspace brought us here, process the information it obtained
          ...
          if (workspace._printAfterPersist) {
            // Add modelName to model for _printModels obj (for use is finding the correct printer)
        ...
        if (i === models.length) {
          if (data[0]) {
            ...
            dispOptions.success = function () {
              if (that._printModels) { that.print(); }
