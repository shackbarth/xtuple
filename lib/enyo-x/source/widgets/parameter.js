  ...
  */
  enyo.kind(enyo.mixin(/** @lends XV.ParameterWidget# */{
    ...
    columnsChanged: function (inSender, inEvent) {
      inEvent = {attr: inEvent.originator.attr,
                show: inEvent.originator.value};
    ...
    buildColumnList: function (attributes, currentColumns) {
      this.$.layoutForm.addColumns(attributes, currentColumns);
    },
    /**
  ...
  enyo.kind({
    ...
    components: [
      ...
      {kind: "XV.TopDrawer", name: "layoutTopDrawer", ontap: "activateLayout", content: "_columns".loc()},
      {kind: "onyx.Drawer", name: "columnsDrawer", open: false, animated: true}
    ],
    /**
    ...
    */
    ...
    /**
      This loops through an array of attributes and creates a formatted
      ToggleButton Widget for each. If the attributes match values
      in the currentSelections, those widgets are toggled on. If the
      attributes match values that are not flagged for layout, they are
      disabled.
    */
    addColumns: function (attributes, currentListItems) {
      var matchSelections = function (sel) { return sel.attr === attributes[i]; },
        value,
        disabled,
        label,
        component;
      // reset this drawer before adding new toggles
      this.$.columnsDrawer.destroyClientControls();
      for (var i = 0; i < attributes.length; i++) {
        // check to see if this attribute is already selected
        value = _.find(currentListItems, matchSelections);
        // check to see if this attribute can be toggled
        disabled = value ? !value.isLayout : false;
        label = this.getColumnString(attributes[i]);
        component = {kind: "XV.ToggleButtonWidget",
          name: attributes[i],
          attr: attributes[i],
          label: label,
        ...
        this.$.columnsDrawer.createComponent(component);
      }
      this.$.columnsDrawer.render();
    },
    /**
      Format the attributes to be in the form of
      translateable strings.
    */
    getColumnString: function (attribute) {
      if (attribute.indexOf('.') !== -1) {
        var attrs = attribute.split("."),
          attrString = ("_" + attrs[0]).loc() + " " +
            ("_" + attrs[1]).loc();
        return attrString;
      }
      return ("_" + attribute).loc();
    },
    /**
    ...
    */
    valueChanged: function (inSender, inEvent) {
      if (inSender.kind === "XV.ToggleButton") {
        inEvent = {attr: inEvent.originator.attr,
          show: inEvent.originator.value};
        this.doColumnsChange(inEvent);
      }
