...
(function () {
  ...
  */
  enyo.kind(enyo.mixin(/** @lends XV.ParameterWidget# */{
    ...
    handlers: {
      ...
      onColumnsChange: "columnsChanged"
    },
    published: {
    ...
    isAllSetUp: false,
    /**
     This bubbles up a columns change event to the
     parent (Navigator).
     */
    columnsChanged: function (inSender, inEvent) {
      // TODO: refresh tree?
    },
    /**
    ...
    create: function () {
      ...
      if (this.getShowSaveFilter()) {
        ...
        // TODO: this will populate both filters and layouts
        this.populateFromUserPref();
      }
      if (this.getShowLayout()) {
        this.createComponent({kind: "XV.LayoutForm", addBefore: null});
      }
    ...
    clearParameters: function () {
      _.each(this.$, function (item) {
        if (item instanceof XV.ParameterItem && item.showing) {
          item.$.input.clear();
          item.doParameterChange({value: null, originator: item});
        }
      });
    },
    /**
    ...
    populateParameters: function (params) {
      ...
      _.each(params, function (value, key) {
        ...
        if (param) {
          ...
          param.doParameterChange({value: value, originator: param});
        }
      }, this);
    },
    /**
  ...
  enyo.kind({
    ...
    components: [
      ...
      {kind: "XV.LayoutPicker", label: "_layout".loc()},
      ...
      {kind: "XV.TopDrawer", name: "saveLayoutTopDrawer", ontap: "activateSaveLayout", content: "_saveLayout".loc()},
      {kind: "onyx.Drawer", name: "saveLayoutDrawer", open: false, animated: true, components: [
        {kind: "onyx.GroupboxHeader", content: "_saveLayout".loc()},
        {kind: "XV.InputWidget", name: "layoutName", label: "_layoutName".loc()},
        {kind: "XV.CheckboxWidget", name: "isLayoutShared", attr: "isLayoutShared", label: "_isShared".loc()},
        {kind: "FittableColumns", classes: "button-row", components: [
          {kind: "onyx.Button", name: "apply", disabled: true, content: "_apply".loc(), ontap: ""},
          {kind: "onyx.Button", content: "_cancel".loc(), ontap: "activateSaveLayout"}
        ]}
      ]},
    ],
    /**
      Opens the columns drawer and switches the drawer icons.
    */
    ...
    */
    activateSaveLayout: function (inSender, inEvent) {
      this.$.saveLayoutDrawer.setOpen(!this.$.saveLayoutDrawer.open);
      this.$.saveLayoutTopDrawer.changeIcon(this.$.saveLayoutDrawer.open);
    },
    /**
    ...
    */
    ...
    /**
      ...
      save layout drawer. It checks for an existing saved and
      shows the warning popup or continues to the save.
    */
    checkExisting: function (inSender, inEvent) {
      var name = this.$.layoutName.getValue(),
        exists = this.layoutExists(name);
      if (exists) {
        //this.$.existingPopup.show();
      } else {
        this.saveLayout(inSender, inEvent);
      }
    },
    /**
      TODO: check for existing layout
    */
    layoutExists: function (name) {
      return false;
    },
    /**
      This is called by the save/apply button in the
      save layout drawer. It bubbles up the event to the parameter
      widget with a callback so it knows when the save was
      successful. It then closes the drawer.
    */
    saveLayout: function (inSender, inEvent) {
      var name = this.$.layoutName.getValue(),
        shared = this.$.isLayoutShared.getValue(),
        exists = this.layoutExists(name),
        that = this;

      inEvent = {model: exists, layoutName: name, isLayoutShared: shared,
        callback: function (model) {
          if (!exists) {
            // TODO: Add layout to a list
            // that.$.LayoutList.getValue().add(model);
          }
          that.activateLayoutSave(); // close the save drawer
          that.$.existingPopup.hide(); // hide the popup if it was showing
          //that.collectionChanged();
        }};
      this.doLayoutSave(inEvent);
    },
  });
  ...
  */
  ...
  /**
    This is a generic editor form for the parameter widget that includes
    a picker for selecting user preference items, a save drawer
    for saving the current item, and a manage drawer for sharing and deleting items.
  */
  enyo.kind(
    /** @lends XV.UserItemForm# */{
    ...
    components: [
      {kind: "onyx.GroupboxHeader", content: "_filters".loc()},
      {kind: "XV.FilterPicker", label: "_filter".loc()},
      {kind: "XV.TopDrawer", name: "saveTopDrawer", ontap: "activateSave", content: "_saveFilters".loc()},
      {kind: "onyx.Drawer", name: "saveDrawer", open: false, animated: true, components: [
        {kind: "onyx.GroupboxHeader", content: "_saveFilter".loc()},
      ...
      {kind: "onyx.Drawer", name: "manageDrawer", open: false, animated: true, components: [
        {kind: "onyx.GroupboxHeader", content: "_manageFilters".loc()},
        {kind: "XV.FilterList", name: "itemList", allowFilter: false},
        {kind: "FittableColumns", classes: "button-row", components: [
          {kind: "onyx.Button", content: "_done".loc(), ontap: "activateManage"}
        ]}
      ]},
      {kind: "onyx.Popup", name: "existingPopup", centered: true,
        ...
        {content: "_editFilter?".loc()},
        {tag: "br"},
        {kind: "onyx.Button", content: "_edit".loc(), ontap: "save" + this.itemType,
          classes: "onyx-blue xv-popup-button"},
        {kind: "onyx.Button", content: "_cancel".loc(), ontap: "hidePopup",
          classes: "xv-popup-button"}
      ]},
    ],
    capitalizeItem: function () {
    ...
    */
    ...
    /**
      Sets the default picker and the
      list that can be managed.
    */
    create: function () {
      this.inherited(arguments);
      this.$.apply.setDisabled(!this.validate());
      ...
      // filters the manage list
      var query =  this.$.itemList.getQuery() || {};
      query.parameters = [];
      query.parameters = [{
        attribute: "createdBy",
        operator: "=",
      ...
      this.$.itemList.setQuery(query);
      this.$.itemList.fetch();
    },
    /**
      Looks at the list of items for the current user and determines
      if an item exists by name.
    */
    itemExists: function (name) {
      var exists = _.find(this.$.itemList.getValue().models,
        function (model) {
          return model.get("name") === name;
        });
      return exists;
    ...
    */
    saveItem: function (inSender, inEvent) {
      ...
      inEvent = {model: exists, filterName: name, isShared: shared,
        callback: function (model) {
          if (!exists) {
            that.$.itemList.getValue().add(model);
          }
          that.activateSave(); // close the save drawer
          that.$.existingPopup.hide(); // hide the popup if it was showing
          that.collectionChanged();
