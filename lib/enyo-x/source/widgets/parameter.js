  ...
  enyo.kind(enyo.mixin(/** @lends XV.ParameterWidget# */{
    ...
    */
    saveFilter: function (inSender, inEvent) {
      ...
      console.log(inEvent.filterName);
      console.log(inEvent.isShared);
    ...
    */
    loadFilter: function (inSender, inEvent) {
      var model = inEvent.model, params = model.get("params");
    ...
    */
    populateParameters: function (params) {
      ...
      _.each(params, function (name) {
        var item, savedValue;
        savedValue = params[name];
        item = this.$[name];
        // TODO: **this might be an issue because it
        // only replaces the value if it's not falsy
        if (item) {
  ...
  enyo.kind({
    ...
    components: [
      ...
      {kind: "onyx.Drawer", name: "manageDrawer", open: false, animated: true, components: [
        ...
        // TODO: Add a list to hold filters
        {kind: "XV.List", components: [
    ...
    activateSave: function (inSender, inEvent) {
      var isOpen = this.$.saveDrawer.open;
      this.$.saveDrawer.setOpen(!isOpen);
      inSender.changeIcon(!isOpen);
    },
    activateManage: function (inSender, inEvent) {
      // TODO: load list with filters
      var isOpen = this.$.manageDrawer.open;
      this.$.manageDrawer.setOpen(!isOpen);
      inSender.changeIcon(!isOpen);
    },
    saveFilter: function (inSender, inEvent) {
