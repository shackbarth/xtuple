  ...
  enyo.kind(enyo.mixin(/** @lends XV.ParameterWidget# */{
    ...
    */
    ...
    populateFromUserPref: function () {
      var filter,
        kind = this.kind,
        lastFilter = XT.session.preferences.get("LastFilterSelected");

      // if we have the JSON dump of filters, find the one for this kind
      if (lastFilter) {
        filter = _.find(lastFilter, function (f) {
          return f.get("kind") === kind;
        });
      }

      // if there is a last filter for
      if (filter) {
        this.$.filterForm.$.filterPicker.setValue(filter.uuid);
      }
