  ...
  enyo.kind(enyo.mixin(/** @lends XV.ParameterWidget# */{
    ...
    */
    loadFilter: function (inSender, inEvent) {
      ...
      // save this filter to the cookie
      dbName = XT.session.details.organization;
      cookieName = 'advancedSearchCache_' + dbName + '_' + this.name;
      enyo.setCookie(cookieName, JSON.stringify(model));
    },
    /**
    ...
    */
    ...
    /**
      Reads the last filter value from the saved cooke
      object and populates the filter picker with this
      value.
     */
    populateFromCookie: function () {
      if (!this.getMemoizeEnabled()) { return false; }
      ...
      this.$.filterForm.$.filterPicker.setValue(cookieValue.uuid);
    },
    /**
  ...
  enyo.kind({
    ...
    */
    ...
    create: function () {
      ...
      var options = {},
        query =  this.$.filterList.getQuery() || {};
      query.parameters = [];
      query.parameters = [{
        attribute: "createdBy",
        operator: "=",
