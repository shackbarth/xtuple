...
(function () {
  ...
  enyo.kind(/** @lends XV.Completer# */{
    ...
    */
    buildList: function (key, value, models, sidecarKey) {
      ...
      if (models && models.length) {
        list = _.filter(models, function (model) {
          var item = model.getValue(key) || "";
          return item.match(regexp);
        });

        for (i = 0; i < list.length; i++) {
          model = list[i];
          item = model.getValue(key);
          if (item) {
            itemContent = model.getValue(key);
            if (sidecarKey) {
              // XXX aesthetics are not my strong point
              itemContent = itemContent + " | " + model.getValue(sidecarKey);
            }
