...
(function () {
  ...
  st.prototype.didCompleteCache = function (cache) {
    var recordType, storeCollection;

    //console.log("TASK COMPLETED: ", this.get("taskName"), cache);
    if (cache.isAutoSync) {
      recordType = cache.model.prototype.recordType;
      // TODO: easier way to get this?
      ...
      // statusChange is a workable proxy for things changing in the collection,
      // but as a binding it gets called more frequently than we need it to.
  ...
  st.prototype.didComplete = function (autoSync) {
    this.set("isComplete", true);
    XT.getStartupManager().taskDidComplete(this);
  };
