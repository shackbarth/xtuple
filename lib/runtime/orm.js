...
XT.ormInstaller = XT.Object.create(
  ...
  /** TEMPORARY!!!!!! */
  targetDatabase: XT.opts.datasource.organization,
  ...
  */
  ...
  init: function() {
    ...
    var root = this.rootDir = _path.resolve(base, '../database/orm');
    var self = this;

    if (XT.typeOf(XT.opts.orm.debugging) !== XT.T_UNDEFINED)
      this.set('debugging', XT.opts.orm.debugging);
  ...
  /**
    When a socket connects to the dev server for the correct
  ...
  */
  ...
  generateModelFiles: function(socket, list, subclass) {
    ...
    for(i=0; i<list.length; ++i) {
      ...
      writePath = _path.join(orm.writePath, filename);
      filename = writePath;
      ...
      if(models[list[i]].patches) {
        var ipatches = models[list[i]].patches;
        for(var key in ipatches) {
          filename = orm.type.d().pre('_').suf('.js');
          writePath = _path.join(key, filename);
          filename = writePath;
          patches[filename] = ipatches[key];
        }
        names.push(filename);
      }
    }
    XT.modelGenerator.writeFiles(gen, true, socket);
    if(patches) XT.modelGenerator.writeFiles(patches, true, socket, true);
    if(subs) XT.modelGenerator.writeFiles(subs, false, socket);
  ...
  */ 
  fileSystemMonitor: function(monitor) {
    ...
    monitor.on('changed', function(file, curr, prev) {
      if(XT.ext(file) !== 'json') return;
      if(last.changed === file) return last.changed = null;
      last.changed = file;
      file = XT.shorten(file, 3);
  ...
  /**
    ...
    @returns {Boolean} true|false if the file/directory should be included.
  */
  ...
  */
  ...
  /**
    ...
    If the noEmitToClient flag is set to true, it will instead
  ...
  */
  ...
  /**
    ...
    @returns {Boolean} true|false if ORM should be enabled.
  */
