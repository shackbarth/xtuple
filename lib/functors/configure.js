...
(function () {
  ...
  X.Functor.create({
     ...
     */
    getAllJsonKeys: function (json, prefix, keys) {
      var key, newPrefix;
      ...
      if (typeof json === 'object') {
        for (key in json) {
          if (json.hasOwnProperty(key)) {
            ...
            this.getAllJsonKeys(json[key], newPrefix, keys);
          }
        }

      } else {
        // add this key unless it's already added
        if (keys.indexOf(prefix) < 0) {
          keys.push(prefix);
        }
      }
      return keys;
    ...
    jsonToCsv: function (results) {
      var i,
        ...
        keys = X._.without(this.getAllJsonKeys(results, '', []), 'id', 'type', 'dataState'),
        row,
        key,
        value,
        recursingValue,
