      ...
      session.once("isReady", function () {
        that.queryForCsv(session, query, function (err, res) {
          if (err) {
            ...
            // TODO: we could set the filename here if we want using something like

            //response.writeHead(200, {"Content-Type": contentType, "Content-Disposition": "attachment; filename = %@".f(filename) });
            response.writeHead(200, {"Content-Type": contentType});
            response.write(res);
            response.end();
