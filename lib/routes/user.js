...
(function () {
  ...
  X.userRoute = X.AdministrativeRoute.create({
    ...
    handle: function (xtr) {
      ...
      if (method === "PUT") {
          return xtr.error({isError: true, reason: "Deprecated functionality"});
        ...
        if (!this.fetch) {
          // this and similar guard statements are a temporary measure to
          // hold us off for the big bullpup refactor. Much of this code
          // is from the node-router and is no longer in legitimate use.
          return xtr.error({isError: true, reason: "Deprecated functionality"});
        }
