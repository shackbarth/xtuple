  ...
  */
  XT.Orm.createView = function (orm) {
    ...
    processOrm = function(orm) {
      ...
      /* process triggers */
      if (orm.triggers && orm.triggers.length) {
        triggers = triggers.concat(orm.triggers);
      }
    ...
    /* Add orm defined triggers on orm generated view */
    query = 'create trigger {triggerName} instead of insert or update or delete on {view} for each row execute procedure {triggerFunction}();';
    for (i = 0; i < triggers.length; i++) {
      plv8.execute(query.replace("{triggerName}", triggers[i].afterDot())
                        .replace("{view}", viewName)
                        .replace("{triggerFunction}", triggers[i]));
    }
