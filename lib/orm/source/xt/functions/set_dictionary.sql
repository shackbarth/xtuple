drop function if exists xt.set_dictionary(text, text);
...
create or replace function xt.set_dictionary(strings text, context text) 
  ...
  count = plv8.execute(sqlCount, [extensionId, isDatabase])[0].count;
  if(count > 0) {
    plv8.execute(sqlUpdate, [strings, extensionId, isDatabase]);
  } else {
    plv8.execute(sqlInsert, [strings, extensionId, isDatabase]);
  }
