...
/** @class */
Xbo.Controller = XM.Object.create(
  /** @scope Xbo.prototype */ {
  ...
  */
  isLoaded: function(request) {
    return this.get("loadedXbos").indexOf(request >= 0);
  },
  ...
  */
  registerPatch: function(patch, target) {
    var q = this.get("queuedPatches"),
        name;
    if (SC.typeOf(target) === SC.T_OBJECT) name = target.name;
    else name = target;
  ...
  _statusBinding: SC.Binding.from("XM.Session.isActive").oneWay(),
  ...
  init: function() {
    ...
    this._info = XM.__LOADABLEINFO__;
  ...
  _invokeCallbacks: function(callbacks, request) {
    ...
    for (; i<callbacks.length; ++i) callbacks[i](xbo); 
  },
