  ...
  (function () {
    ...
    initSocket = function (socket) {
      ...
      console.log("thanks for connecting to me");
    };
    ...
    findExisting = function (nameSpace, type) {
     ...
     var installCopy = function (socket, ack) {
        console.log("installCopy", arguments);
      ...
      var selectCopy =  function (socket, options, ack) {
    ...
    var refreshCopy = function (socket, options, ack) {
      ...
      console.log(socket.databaseOptions);
      X.db.query(sql, socket.databaseOptions, callback);
    };

  exports.refresh = refreshCopy;
  exports.select = selectCopy;
  ...
  X.Server.create({
    ...
    install: function (socket, ack) {
      console.log("install", arguments, socket.orms.XM);
      var valid = [], installer = _.bind(installQueue, this, socket, ack), orms;
      orms = socket.orms;
      _.each(orms, function (namespace) {
        _.each(namespace, function (orm) {
    ...
    refresh: function (socket, options, ack) {
      ...
      if (X.typeOf(files[0]) === X.T_HASH) {
        ...
        errors = errors.errors;
        _.each(errors, function (error) {
          console.log("message", "failed to parse %@: %@".f(X.shorten(error.file, 4), error.message));
        });
      }
