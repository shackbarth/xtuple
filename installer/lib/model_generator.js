...
XT.MODELSDIRECTORY = '../../client/frameworks/xm/packages';
XT.SUBMODELSDIRECTORY = '../../generated_template_models';
...
XT.modelGenerator = XT.Object.create(
  ...
  generate: function(orm, superclass) {
    ...
    var namespace = orm.nameSpace || '',
        type = orm.type || '',
        date = new Date(), 
        body = superclass ? (orm.isExtension ? this.__patchtpl : this.__mixintpl) : this.__subtpl,
        attributes, nestedRecordNamespace;

    attributes = this.buildAttributes(orm).join(',\n');
    ...
    if(orm.isExtension) attributes = attributes.replace(/\n/g,'\n  ');
    body = body.f({
  ...
  writeFiles: function(models, superclass, socket, patch) {
    ...
    for(i=0; i<files.length; ++i) {
      ...
      patch ? parts.push('patches') : parts.push('mixins');
      parts.push(end); 
  ...
  buildAttributes: function(orm) {
    ...
    for(i=0; i<orm.properties.length; ++i) {
      ...
      if(obj.defaultValue !== undefined && obj.defaultValue !== null) {
        ...
        } else if (obj.defaultValue === 'startOfTime') {
          def = 'defaultValue: function() {\n      return XT.DateTime.startOfTime();\n    }';
        } else if (obj.defaultValue === 'endOfTime') {
