  ...
  */
  ...
  var moneyTransform = {
    insertTargets: function(obj, alias) {
      ...
      if(amount && amount.isMaster) ret.push(amount.column);
      if(currency && currency.isMaster) ret.push(currency.column);
      if(effective && effective.isMaster) ret.push(effective.column);
      if(rate && rate.isMaster) ret.push(rate.column);
    ...
    insertSources: function(obj, alias) {
     ...
     if(amount && amount.isMaster) ret.push('(new.' + alias + ').amount');
     if(currency && currency.isMaster) ret.push('(new.' + alias + ').currency');
     if(effective && effective.isMaster) ret.push('(new.' + alias + ').effective');
     if(rate && rate.isMaster) ret.insertSources.push('(new.' + alias + ').rate');
   ...
   selectColumn: function(obj, alias, table, nameSpace) {
      ...
      ret = ret.replace(/{amount}/, amtcol)
               ...
               .replace(/{nameSpace}/, nameSpace.decamelize())
               .replace(/{type}/, type)
               .replace(/{alias}/, alias);
   ...
   updateExpressions: function(obj, alias) {
     var ret = [], 
      ...
      if(amount && amount.isMaster) ret.push(amount.column + ' = (new.' + alias + ').amount'); 
      if(currency && currency.isMaster) ret.push(currency.column + ' = (new.' + alias + ').currency'); 
      if(effective && effective.isMaster) ret.push(effective.column + ' = (new.' + alias + ').effective');
      if(rate && rate.isMaster) ret.push(rate.column + ' = (new.' + alias + ').rate'); 

      return ret;      
    }
  }
  
  XT.Orm.registerTransform('Money', moneyTransform);
  XT.Orm.registerTransform('Cost', moneyTransform);
  XT.Orm.registerTransform('SalesPrice', moneyTransform);
  XT.Orm.registerTransform('PurchPrice', moneyTransform);
  XT.Orm.registerTransform('ExtPrice', moneyTransform);
