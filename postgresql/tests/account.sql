...
values (
  99999, 'TEST NUMBER', 'TEST NAME', true, 'O', (select user_account_info from xm.user_account_info where username='admin'), NULL, 'TEST NOTES', 
    (select contact_info from xm.contact_info where id =1), (select contact_info from xm.contact_info where id =6));
...
values (
  99999, 99999, (select characteristic from xm.characteristic where id=16), 'TEST XM.ACCOUNT_CHARACTERISTIC VIEW INSERT');

insert into xm.item_assignment (
  id, source, source_type, item, purpose)
values (
  99999, 99999, 'CRMA', (select item_info from xm.item_info where number = 'BTRUCK1'), 'S');

insert into xm.contact_assignment (
  id, source, source_type, contact, purpose)
values (
  99998, 99999, 'CRMA', (select contact_info from xm.contact_info where id = 2), 'S');
  
insert into xm.image_assignment (
  id, source, source_type, image, purpose)
values (
  99997, 99999, 'CRMA', (select image_info from xm.image_info order by id limit 1), 'S');
...
update xm.account set
  ...
  "type" = 'I',
  owner = null,
  parent = (select account_info from xm.account_info where id=1),
  notes = '**UPDATED NOTES**',
  primary_contact = (select contact_info from xm.contact_info where id=2),
  secondary_contact = (select contact_info from xm.contact_info where id=7)
where id = 99999;
...
update xm.account_characteristic set
  characteristic = (select characteristic from xm.characteristic where id=12),
