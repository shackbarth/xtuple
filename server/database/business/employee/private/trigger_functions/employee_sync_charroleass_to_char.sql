create or replace function private.employee_sync_charroleass_to_char() returns trigger as $$
declare
  datatype text;
  id integer;
  state boolean;
begin
  ...
  if ( datatype = 'Employee' ) then
    ...
    alter table public.char disable trigger "employee_sync_char_to_charroleass";
    
    update public.char set
      char_employees = state
    where ( char_id = id );

    -- clean up
  
    alter table public.char enable trigger "employee_sync_char_to_charroleass";
