create or replace function private.account_sync_charroleass_to_char() returns trigger as $$
declare
  datatype text;
  id integer;
  state boolean;
begin
  ...
  if ( datatype = 'Account' ) then
    ...
    alter table public.char disable trigger "account_sync_char_to_charroleass";
    
    update public.char set
      char_crmaccounts = state
    where ( char_id = id );

    -- clean up

    alter table public.char enable trigger "account_sync_char_to_charroleass";
