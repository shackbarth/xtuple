create or replace function private.customer_sync_charroleass_to_char() returns trigger as $$
declare
  datatype text;
  id integer;
  state boolean;
begin
  ...
  if ( datatype = 'Customer' ) then
    ...
    alter table public.char disable trigger "customer_sync_char_to_charroleass";
  
    update public.char set
      char_customers = state
    where ( char_id = id );

    alter table public.char enable trigger "customer_sync_char_to_charroleass";
