create or replace function private.create_model_view(m_name text) returns void as $$
...
declare
  ...
  clauses text[] = '{}';
  order_by text[] = '{}';
  comments text = 'System view generated by model tables: WARNING! Do not make changes, add rules or dependencies directly to this view!';
  rules text[] = '{}';
  query text;
  rec record;
  s_name text;
begin
  ...
  -- Loop through model definition records and build components
  i := 1;

  for rec in
    -- base model
    select model_id, model_columns, model_schema_name,
  ...
  loop
    ...
    order_by := order_by || rec.model_order;
...
'  || rec.model_comment;
  ...
  if array_length(order_by, 1) > 0 then
    query := query || ' order by ' || array_to_string(order_by, ' , ');
  end if;
